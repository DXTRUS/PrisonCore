...
public final class RandomSelector<T> {
    ...
    /**
     * Creates a random selector among <tt>elements</tt> where the elements have a
     * weight defined by <tt>weighter</tt>.
     *
     * <p>
     * A copy of <tt>elements</tt> is kept, so any modification to <tt>elements</tt>
     * will not be reflected in returned values.
    ...
    public static <T> RandomSelector<T> weighted(final Collection<T> elements,
        ...
        for (int i = 0; i < size; i++) {
            ...
            Preconditions.checkArgument(weight > 0d, "weighter returned a negative number");
    ...
    /**
     * Returns a stream of elements using <tt>random</tt>. The stream must use a
     * terminal operation to become closed and free the resources it's been using.
     *
     * <p>
